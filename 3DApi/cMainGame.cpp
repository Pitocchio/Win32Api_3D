...
cMainGame::~cMainGame()
{
	SelectObject(m_MemDC, m_hOldBtimap);
...
void cMainGame::Setup()
{
	...
	m_hOldBtimap = (HBITMAP)SelectObject(m_MemDC, m_hBitmap);
	ReleaseDC(g_hWnd, hdc);

	m_vecVertex.push_back(cVector3(-1.0f, -1.0f, -1.0f)); // 0번
	m_vecVertex.push_back(cVector3(-1.0f,  1.0f, -1.0f)); // 1번
	m_vecVertex.push_back(cVector3( 1.0f,  1.0f, -1.0f)); // 2번
	m_vecVertex.push_back(cVector3( 1.0f, -1.0f, -1.0f)); // 3번

	m_vecVertex.push_back(cVector3(-1.0f, -1.0f,  1.0f)); // 4번
	m_vecVertex.push_back(cVector3(-1.0f,  1.0f,  1.0f)); // 5번
	m_vecVertex.push_back(cVector3( 1.0f,  1.0f,  1.0f)); // 6번
	m_vecVertex.push_back(cVector3( 1.0f, -1.0f,  1.0f)); // 7번
...
void cMainGame::Update()
{
	...
	m_matProj = cMatrix::Projection( PI/4.0f,
		(float)rc.right / (float)rc.bottom, 1.0f, 1000.0f); //near, far 를 의미near와 far 사이에있는것들만 그려줄것
...
void cMainGame::Render(HDC hdc)
{
	...
	PatBlt(m_MemDC,
		rc.left, rc.top, rc.right, rc.bottom, WHITENESS);
...
bool cMainGame::IsBackFace(cVector3& v1, cVector3& v2, cVector3& v3) // 백페이스 컬링
{
	cVector3 v12 = v2 - v1;
	cVector3 v13 = v3 - v1;

	cVector3 n = cVector3::Cross(v12, v13); // 노말벡터
	cVector3 l = cVector3(0.0f, 0.0f, 1.0f); // 룩벡터
